...
<body>
    ...
    <header class="form-ver">
        <span>
            Form : VP-ENG-02A Date : 12.11.24 Rev.3
        </span>
    </header>
    ...
    @php
        ...
        if (!is_array($taskChunks)) {
            $taskChunks = [];
        }
        $totalPages = $totalChunks > 0 ? ceil($totalChunks / $eachtaskChunk) : 1; // Avoid division by zero
    @endphp

    {{--
    <h4>Task Chunks Content:</h4>
    <pre>{{ print_r($taskChunks, true) }}</pre> <!-- Display the content of taskChunks --> --}}
    ...
    @foreach ($taskChunks as $index => $chunk)
        ...
        <div class="page">
                ...
                <thead class="border-left-bold border-right-bold">
                    ...
                    <tr>
                        ...
                        <th colspan="5" class="th-0 text-start">
                            {{ convertTime($loadDataWS->arrival_time_ws) }}</th>
                    </tr>
                    <tr>
                        <th colspan="4" class="th-0 text-start fit-content th-date">DATE<br>(TANGGAL)</th>
                        <th colspan="1" class="th-0 text-center cell-fit tikdu_eq">:</th>
                        <th colspan="4" class="th-0 text-start fit-content">
                            {{ convertDate($loadDataWS->working_date_ws) }}</th>
                        <th colspan="1" class="th-0 text-center cell-fit text-trans tikdu_eq">:</th>
                        <th colspan="1" class="th-0 text-start fin-th">FINISH TIME<br>(WAKTU SELESAI)</th>
                        <th colspan="1" class="th-0 text-center cell-fit tikdu_eq">:</th>
                        <th colspan="5" class="th-0 text-start">
                            {{ convertTime($loadDataWS->finish_time_ws) }}</th>
                    </tr>
                    <tr>
                    ...
                    @foreach ($chunk as $relDWS)
                        <tr>
                            <td colspan="3" class="text-center align-top cell-fit time-th">
                                {{ convertTimeV2($relDWS['start_time_task']) }}</td>
                            ...
                            <td class="text-center align-top pro-act" colspan="3">
                                ...
                                @if ($relDWS['monitor']['qty'])
                                    @php
                                        ...
                                        $up = $relatedTasks->count() > 0 ? $totalProgress / $relatedTasks->count() : 0;
            ...
            <footer class="form-footer">
                @php
                    $currentPage = floor($index / $eachtaskChunk) + 1; // Avoid division by zero
                @endphp
                <span>
