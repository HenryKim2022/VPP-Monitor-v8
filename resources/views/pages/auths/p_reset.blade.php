                    ...
                    <div class="auth-inner row m-0">
                        ...
                        <div class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-5">
                            <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
                                ...
                                <p class="card-text mb-2 text-center">Please submit your email to receive reset
                                    password link.</p>
                                <form class="auth-login-form mt-2" action="{{ route('reset.send') }}"
                                    method="POST">
                                    @csrf
                                    <div class="form-group">
                                        <label class="form-label" for="user-email">Email</label>
                                        <input class="form-control" id="user-email" type="text" name="user-email"
                                            placeholder="youremail@example.com" aria-describedby="user-email"
                                            autofocus="" tabindex="1"
                                            value="{{ old('user-email', $inputs['user-email'] ?? '') }}" />
                                    </div>

                                    <button class="btn mail-token btn-primary btn-block" tabindex="4">Send Reset
                                        Link</button>
                                </form>


                                <p class="text-center mt-2"><a href="{{ route('login.page') }}"><i
                                            data-feather="chevron-left"></i> Back to login</a></p>

                                <script>
                                    const mailingTokenBtn = document.querySelector('.mail-token'); // Corrected selector

                                    mailingTokenBtn.addEventListener('click', function() {
                                        const preloader = document.getElementById('preloader');
